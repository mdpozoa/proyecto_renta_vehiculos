...
const MODULE_NAME = 'Detalle Factura';

const fields = [
  { key: 'ID_Detalle',            label: 'ID Detalle',     type: 'number',  readOnly: true },
  { key: 'ID_Factura',            label: 'ID Factura',     type: 'number',  required: true },
  { key: 'Descripcion_Detalle',   label: 'Descripción',    type: 'text',    required: true, fullWidth: true },
  { key: 'Cantidad_Detalle',      label: 'Cantidad',       type: 'number',  required: true, min: 1 },
  { key: 'Precio_Unitario_Detalle',label: 'Precio Unitario ($)', type: 'decimal', required: true },
  { key: 'Subtotal_Detalle',      label: 'Subtotal ($)',   type: 'decimal', required: true },
];
...
<template>
  <div class="crud-view">
    <div class="page-header">
      <h1 class="page-title text-primary">Módulo Detalle Facturas</h1>
      <p class="text-muted">Gestiona las líneas de detalle de cada factura.</p>
    </div>
    <div style="display:flex; justify-content:flex-end; margin-bottom:2rem;">
      <button @click="openCreate" class="btn-primary" style="padding:0.75rem 2rem; display:flex; align-items:center; gap:0.5rem;">
        <span style="font-size:1.25rem;">+</span> Nuevo Detalle
      </button>
    </div>
    <div class="glass-panel" style="padding:0; overflow:hidden;">
      ...
      <div v-else style="padding:4rem; text-align:center;"><h3 class="text-muted">Sin detalles registrados</h3><button @click="openCreate" class="btn-primary" style="margin-top:2rem;">Crear primer detalle</button></div>
    </div>
