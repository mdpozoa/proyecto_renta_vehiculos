...
const MODULE_NAME = 'Ciudad';

const fields = [
  { key: 'ID_Ciudad',       label: 'ID Ciudad',   type: 'number', readOnly: true },
  { key: 'Nombre_Ciudad',   label: 'Nombre',      type: 'text',   required: true },
  { key: 'Provincia_Ciudad',label: 'Provincia',   type: 'text',   required: true },
];
...
<template>
  <div class="crud-view">
    <div class="page-header">
      <h1 class="page-title text-primary">Módulo Ciudades</h1>
      <p class="text-muted">Gestiona las ciudades disponibles en el sistema.</p>
    </div>
    ...
    <div style="display:flex; justify-content:flex-end; margin-bottom:2rem;">
      <button @click="openCreate" class="btn-primary" style="padding:0.75rem 2rem; display:flex; align-items:center; gap:0.5rem;">
        <span style="font-size:1.25rem;">+</span> Nueva Ciudad
      </button>
    </div>

    <div class="glass-panel" style="padding:0; overflow:hidden;">
      <div v-if="loading" style="padding:4rem; text-align:center; color:var(--primary);">
        <h2 style="animation:pulse 1.5s infinite alternate;">⏳ Cargando...</h2>
      </div>
