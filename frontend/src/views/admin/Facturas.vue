...
const MODULE_NAME = 'Factura';

const fields = [
  { key: 'ID_Factura',       label: 'ID Factura',       type: 'number',  readOnly: true },
  { key: 'ID_Contrato',      label: 'ID Contrato',      type: 'number',  required: true },
  { key: 'ID_Usuario',       label: 'ID Usuario',       type: 'number',  required: true },
  { key: 'Numero_Factura',   label: 'Número Factura',   type: 'text',    required: true },
  { key: 'F_Emision_Factura',label: 'Fecha Emisión',    type: 'date',    required: true },
  { key: 'Subtotal_Factura', label: 'Subtotal ($)',      type: 'decimal', required: true },
  { key: 'IVA_Factura',      label: 'IVA ($)',           type: 'decimal', required: true },
  { key: 'Total_Factura',    label: 'Total ($)',         type: 'decimal', required: true },
  { key: 'M_Pago_Factura',   label: 'Método Pago',      type: 'select',  required: true,
    options: [
...
const deleteItem = async (item) => {
  if (!confirm(`¿Eliminar ${MODULE_NAME} N° ${item['Numero_Factura']}?`)) return;
...
<template>
  <div class="crud-view">
    <div class="page-header">
      <h1 class="page-title text-primary">Módulo Facturas</h1>
      <p class="text-muted">Gestiona las facturas de los contratos de renta.</p>
    </div>
    <div style="display:flex; justify-content:flex-end; margin-bottom:2rem;">
      <button @click="openCreate" class="btn-primary" style="padding:0.75rem 2rem; display:flex; align-items:center; gap:0.5rem;">
        <span style="font-size:1.25rem;">+</span> Nueva Factura
      </button>
    </div>
    <div class="glass-panel" style="padding:0; overflow:hidden;">
      ...
      <div v-else style="padding:4rem; text-align:center;"><h3 class="text-muted">Sin facturas registradas</h3><button @click="openCreate" class="btn-primary" style="margin-top:2rem;">Crear primera factura</button></div>
    </div>
