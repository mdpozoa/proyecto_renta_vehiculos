...
const PK = 'ID_Contrato';
const ENDPOINT = '/Contratos';
const MODULE_NAME = 'Contrato';

const fields = [
  { key: 'ID_Contrato',     label: 'ID Contrato',   type: 'number',   readOnly: true },
  { key: 'ID_Reserva',      label: 'ID Reserva',    type: 'number',   required: true },
  { key: 'Ter_Con_Contrato',label: 'Términos',       type: 'textarea', required: true, fullWidth: true },
  { key: 'Firma_Contrato',  label: 'Firma',          type: 'text',     required: false, fullWidth: true },
];
...
<template>
  <div class="crud-view">
    <div class="page-header">
      <h1 class="page-title text-primary">Módulo Contratos</h1>
      <p class="text-muted">Gestiona los contratos de arrendamiento.</p>
    </div>
    <div style="display:flex; justify-content:flex-end; margin-bottom:2rem;">
      <button @click="openCreate" class="btn-primary" style="padding:0.75rem 2rem; display:flex; align-items:center; gap:0.5rem;">
        <span style="font-size:1.25rem;">+</span> Nuevo Contrato
      </button>
    </div>
    <div class="glass-panel" style="padding:0; overflow:hidden;">
      ...
      <div v-else style="padding:4rem; text-align:center;"><h3 class="text-muted">Sin contratos registrados</h3><button @click="openCreate" class="btn-primary" style="margin-top:2rem;">Crear primer contrato</button></div>
    </div>
